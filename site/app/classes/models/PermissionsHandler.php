...
class PermissionsHandler {
	...
	public function fetchManagePermissionsColumnDefinitions( ) {
		...
		$columns[3] = array( "title" => "Permission Setting", "data" => 3, "orderable" => false, "sortable" => false, "className" => "text-center", "dbCol" => '' );
	...
	public function buildCustomizedPermissionsList( $params ) {
		...
		$query = "SELECT permission_id, permission_name, permission_desc, permission_level FROM " . DB_MAIN . ".permissions";
		$options = array( );
		
		if( isset( $params['search'] ) && strlen($params['search']['value']) > 0 ) {
			$query .= " WHERE permission_id=? OR permission_name LIKE ? OR permission_desc LIKE ?";
	...
	private function buildManagePermissionsOptions( $permInfo ) {
		...
		$options = array( );
		$permissions = array( "public", "observer", "curator", "poweruser", "admin" );
